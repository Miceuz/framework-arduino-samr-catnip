...
#ifndef _VARIANT_RADIOBUS_XENO_
#define _VARIANT_RADIOBUS_XENO_
 ...
 * The coarse calibration value is loaded from NVM OTP (factory calibration
 * values).
 */
...
#define NUM_PIN_DESCRIPTION_ENTRIES (42u)
...
#define digitalPinHasPWM(P)                                                    \
  ((g_APinDescription[P].ulPinAttribute & PIN_ATTR_TIMER_PWM) ==               \
   PIN_ATTR_TIMER_PWM)
...
#define PIN_LED_8 (8u)
#define PIN_LED PIN_LED_8
#define PIN_LED2 PIN_LED_RXL
#define PIN_LED3 PIN_LED_TXL
#define LED_BUILTIN PIN_LED_8
#define PIN_LED_WAN 8
#define PIN_LED_SENS 9
#define PIN_LED_BATT 10

#define PIN_RADIO_CS 29
#define PIN_RADIO_RST 33
#define PIN_RADIO_DIO0 34
#define PIN_RADIO_DIO1 35
#define PIN_RADIO_DIO2 36
#define PIN_RADIO_DIO3 37
#define PIN_RADIO_DIO4 38
#define PIN_RADIO_DIO5 39
#define PIN_RADIO_TXCO_PWR 40
#define PIN_RADIO_BAND_SEL 41
#define PIN_RADIO_SWITCH_PWR 42
...
/* Reference voltage pins (define even if not enabled with PIN_ATTR_REF in the
 * PinDescription table) */

#define REFB_PIN (18ul)
...
static const uint8_t SS =
    PIN_SPI_SS; // The SERCOM SS PAD is available on this pin but HW SS isn't
...
static const uint8_t SS1 =
    PIN_SPI1_SS; // The SERCOM SS PAD is available on this pin but HW SS isn't
                 // used. Set here only for reference.
 ...
 * PIN_USB_HOST_ENABLE_VALUE during startup. Leave undefined to disable this
 * pin.
 */
...
// SERIAL_PORT_MONITOR seems to be used only by the USB Host library (as
// of 1.6.5). It normally allows debugging output on the USB programming port,
// while the USB host uses the USB native port. The programming port is
// connected to a hardware UART through a USB-Serial bridge (EDBG chip) on the
// Zero. Boards that do not have the EDBG chip will have to connect a USB-TTL
// serial adapter to 'Serial' to get the USB Host debugging output.
...
// The Xeno does not have the EDBG support chip, which provides a USB-UART
// bridge accessible using Serial (the Arduino serial monitor is normally
// connected to this). So, the USB virtual serial port (SerialUSB) must be used
// to communicate with the host. Because most sketches use Serial to print to
// the monitor, it is aliased to SerialUSB. Remember to use while(!Serial); to
// wait for a connection before Serial printing. When USB CDC is enabled, Serial
// refers to SerialUSB, otherwise it refers to Serial1.
