...
#if (SAMD21 || SAMD11)
  GCLK->CLKCTRL.reg =
      (GCLK_CLKCTRL_ID(GCM_EIC) | GCLK_CLKCTRL_GEN_GCLK0 | GCLK_CLKCTRL_CLKEN);
...
// SAMD51, SAML, and SAMC EIC.CONFIG registers are enable-protected, thus must
// be disabled/enabled for each config change (see below)
#if (SAMD21 || SAMD11)
  /* Shall we do that?
...
#if (SAML21 || SAMR34)
  // The CHANGE and RISING interrupt modes on pin A31 on the SAML21 do not seem
  // to work properly
  if ((GetPort(pin) == 0) && (GetPin(pin) == 31) &&
      ((mode == CHANGE) || (mode == RISING)))
    return;
#endif
...
void detachInterrupt(uint32_t pin) {
  ...
  // Disable wakeup capability on pin during sleep (WAKEUP always enabled on
  // SAMD51, SAML, and SAMC)
#if (SAMD21 || SAMD11)
  EIC->WAKEUP.reg &= ~(inMask);
#endif
